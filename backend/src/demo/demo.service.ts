  ...
  en: {
    ...
    teams: [
      ...
      },
      {
        key: 'qa',
        name: 'QA',
        description: 'Regression checks, acceptance criteria, and release quality.',
        color: '#0f766e',
      },
      {
        key: 'support',
    ...
    boards: [
      {
        ...
        description: 'Quarterly initiatives, discovery, launch planning, and risk tracking.',
        color: '#7c3aed',
      ...
      },
      {
        key: 'sprint',
        title: 'Sprint Board',
        description: 'Current engineering sprint with ownership, priority, and delivery signals.',
        color: '#2563eb',
      ...
      },
      {
        ...
        description: 'Portfolio review workflow for leads, trial tasks, and interview stages.',
        color: '#0f766e',
      ...
      },
      {
        key: 'support',
        title: 'Support Bugs',
        description: 'Customer-facing bugs, triage, SLA work, and follow-up tasks.',
        color: '#f97316',
  ...
  ru: {
    ...
    teams: [
      ...
      },
      {
        key: 'management',
        name: 'Product',
        description: 'Роадмап, приоритеты, delivery и контекст для стейкхолдеров.',
        color: '#7c3aed',
      },
      {
        key: 'qa',
    ...
    boards: [
      {
        ...
        description: 'Квартальные инициативы, discovery, launch planning и отслеживание рисков.',
        color: '#7c3aed',
        columns: ['Идеи', 'Исследование', 'Запланировано', 'В работе', 'Выпущено'],
        labels: ['roadmap', 'growth', 'research', 'launch', 'risk'],
      },
      {
        key: 'sprint',
        title: 'Спринт',
        description: 'Текущий engineering sprint с владельцами, приоритетами и delivery сигналами.',
        color: '#2563eb',
      ...
      },
      {
        key: 'design',
        title: 'Дизайн Review',
        description: 'UX polish, mobile layouts, дизайн QA и задачи на handoff.',
        color: '#db2777',
        columns: ['Запросы', 'Исследование', 'Прототип', 'Review', 'Подтверждено'],
        labels: ['ux', 'mobile', 'a11y', 'handoff', 'research'],
      },
      {
        key: 'hiring',
        ...
        color: '#0f766e',
        columns: ['Поиск кандидатов', 'Скрининг', 'Собеседование', 'Оффер', 'Наняты'],
        labels: ['people', 'interview', 'portfolio', 'coordination', 'offer'],
      },
      {
        key: 'support',
        ...
        color: '#f97316',
        columns: ['Входящие', 'Распределено', 'В работе', 'Проверка', 'Закрыто'],
        labels: ['bug', 'customer', 'incident', 'sla', 'regression'],
      },
    ],
    taskVerbs: [
...
export class DemoService {
  ...
  private async getDemoStartBoardId(
    ...
    const sprint = boards.find(
      (board) =>
        board.title === sprintTitle || board.title === 'Sprint Board',
    );
  ...
  private async upsertDemoUser(
    ...
    return userRepo.save(
      userRepo.create({
        email: spec.email,
        name: spec.name,
        password: this.config.get<string>('DEMO_USER_PASSWORD') ?? 'demo-user-password',
        avatar,
        avatarProvider: 'dicebear',
  ...
  private async createBoards(
    ...
    for (const spec of boardSpecs) {
      ...
      boards.push(Object.assign(board, { demoKey: spec.key, demoColumns: columns }));
    }
    ...
    boards.forEach((board, boardIndex) => {
      ...
      board.demoColumns.forEach((column, columnIndex) => {
        ...
        for (let order = 0; order < 5; order += 1) {
          const globalIndex =
            boardIndex * 25 + columnIndex * 5 + order;
          const assignee =
            globalIndex % 7 === 0 ? null : allUsers[globalIndex % allUsers.length];
  ...
  private createSavedViews(
    ...
    const sprint = boards.find((board) => board.demoKey === 'sprint') ?? boards[0];
    const design = boards.find((board) => board.demoKey === 'design') ?? boards[0];
    const support = boards.find((board) => board.demoKey === 'support') ?? boards[0];
  ...
  private getDueOffset(index: number, isDoneColumn: boolean) {
    if (isDoneColumn) return -((index % 18) + 3);
    if (index % 13 === 0) return null;
    const offsets = [-9, -4, -1, 0, 2, 5, 9, 14];
